...
uint16_t volatile sample[ADC_SAMPLE_SIZE];
...
void ADC_Init(void)
{
  ...
  // Конфигурация АЦП
  MDR_ADC->ADC1_CFG = (0 << ADC1_CFG_REG_ADON_Pos)     // Работа АЦП (пока отключен)
...
{
  // Переменные для цикла сортировка
  uint32_t i, j, t;

  // Сортировка простыми обменами
  for (i = 0; i < ADC_SAMPLE_SIZE - 3; i++)
  {
    for (j = 0; j < ADC_SAMPLE_SIZE - 2; j++)
    {
  ...
  return sample[ADC_SAMPLE_SIZE / 2];
}
...
void Thread_ADC(void *argument)
{
  ...
  while (1)
  {
    ...
    // Медианная фильтрация полученной выборки
    digit = ADC_MedianFilter(sample);

    // Установка нового значения ширины импульсов
    MDR_TIMER1->CCR1 = PULSE_PERIOD(PULSE_FREQUENCY) * digit / D_MAX;

    // Отображение нового коэфициента заполнения на дисплей
    snprintf(message, LCD_STR_LEN, "       D = %u%%", (digit * 100 / D_MAX));
    LCD_PutString(message, 3);

    // Отображение результата аналого-цифрового преобразования на дисплей
    snprintf(message, LCD_STR_LEN, "     ADC = %u", digit);
