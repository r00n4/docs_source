...
void TIMER_PortCfg(void)
{
  ...
  MDR_PORTF->OE     |=  (1 << 6);   // Направление передачи данных (вывод)
  MDR_PORTF->PULL   &= ~(1 << 6);   // Резистор подтяжки к цепи питания (выключен)
  MDR_PORTF->PULL   &= ~(1 << 22);  // Резистор подтяжки к земле (выключен)
  ...
  MDR_PORTF->PD     &= ~(1 << 22);  // Триггер Шмитта (выключен)
  MDR_PORTF->PWR    |=  (3 << 12);  // Длительность фронтов импульсов (низкая)
  MDR_PORTF->GFEN   &= ~(1 << 6);   // Цифровой фильтр (выключен)
}
