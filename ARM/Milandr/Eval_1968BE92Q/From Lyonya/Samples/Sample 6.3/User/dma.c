...
void DMA_Init(void)
{
  ...
  DMA_ControlData[8].DMA_Control = (1 << DMA_CHANNEL_CFG_CYCLE_CTRL_Pos)                     // Режим работы контроллера DMA (основной)
                                 ...
                                 | (2 << DMA_CHANNEL_CFG_SRC_SIZE_Pos)                       // Размерность данных источника (4 байта)
                                 | (3 << DMA_CHANNEL_CFG_SRC_INC_Pos)                        // Инкремент адреса источника данных (отсутствует)
                                 | (2 << DMA_CHANNEL_CFG_DST_SIZE_Pos)                       // Размерность данных приемника (4 байта)
                                 | (2U << DMA_CHANNEL_CFG_DST_INC_Pos);                      // Инкремент адреса приемника данных (4 байта)
  ...
  DMA_ControlData[8].DMA_DestinationEndPointer = (uint32_t)&sample + ((ADC_SAMPLE_SIZE - 1) << 2);
