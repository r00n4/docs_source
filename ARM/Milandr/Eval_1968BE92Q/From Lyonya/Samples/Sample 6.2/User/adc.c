...
void ADC_PortCfg(void)
{
  ...
  // Конфигурация линии PD6
  MDR_PORTD->OE     &= ~(1 << 6);
  MDR_PORTD->FUNC   &= ~(3 << 12);
  MDR_PORTD->ANALOG &= ~(1 << 6);   // Режим работы линии (аналоговый)
  MDR_PORTD->PULL   &= ~(1 << 6);   // Резистор подтяжки к цепи питания (отключен)
  MDR_PORTD->PULL   &= ~(1 << 22);  // Резистор подтяжки к земле (отключен)
  MDR_PORTD->PD     &= ~(1 << 6);
  MDR_PORTD->PD     &= ~(1 << 22);
  MDR_PORTD->PWR    &= ~(3 << 12);
  MDR_PORTD->GFEN   &= ~(1 << 6);
...
void ADC_Init(void)
{
  ...
  MDR_ADC->ADC1_CFG = (0 << ADC1_CFG_REG_ADON_Pos)     // Работа АЦП (пока отключен)
                    ...
                    | (6 << ADC1_CFG_REG_CHS_Pos)      // Целевой канал преобразователя (ADC6)
...
void Thread_ADC(void *argument)
{
  ...
  float temperature;          // Расчетное значение температуры
  ...
  while (1)
  {
    ...
    // Вычисление температуры на основе результатов преобразования
    temperature = TEMPERATURE(digit);

    // Отображение результата измерения температуры на дисплей (в градусах Цельсия)
